                ...
                {spot.status !== 'empty' ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '12px' }}>
                        ...
                        {spot.status !== 'reserved' ? (
                            ...
                            <button className="btn btn-primary" onClick={() => onRelease(spot.id)}>
                                Release Place
                            </button>
                        )}
                    </div>
                    ...
                    <div style={{ marginTop: '16px' }}>
                        <p style={{ color: 'var(--text-muted)', marginBottom: '16px', fontSize: '0.85rem' }}>Enter Last Name and First Name to reserve this spot.</p>
...
function MultipleReleaseModal({ onClose, onMultipleRelease, blocks }) {
    ...
    const handleRelease = () => {
        const start = parseInt(fromNum);
        const end = parseInt(toNum);
        if (isNaN(start) || isNaN(end) || start > end) { setPendingSpotIds(null); return; }
        const spotIds = [];
        for (let i = start; i <= end; i++) { spotIds.push(`${block}${i}`); }
        setPendingSpotIds(spotIds);
    };

    const handleConfirm = () => { if (pendingSpotIds) onMultipleRelease(pendingSpotIds); };
            ...
            <motion.div className="modal" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()} style={{ maxWidth: '420px' }}>
                ...
                {pendingSpotIds ? (
                    ...
                    <>
                        ...
                        <button className="btn btn-danger" style={{ width: '100%' }} onClick={handleRelease}>Release Range</button>
                    </>
                )}
            </motion.div>
        </motion.div>
    ...
    return (
        <div className="parking-container">
            ...
            <div className="parking-stats-bar">
                ...
                <div className="parking-stat"><div className="stat-dot" style={{ background: 'var(--blue)' }} /><span className="stat-label">Stored VINs</span><span className="stat-value text-blue">{stats.occupied}</span></div>
